...
this patterns returns both an error and a usable value makes it easier for library users
*/
...
func Concat(parts ...string) (string, error) {
	if len(parts) == 0 {
		return "", errors.New("No strings supplied") // return empty string and an error
	}
