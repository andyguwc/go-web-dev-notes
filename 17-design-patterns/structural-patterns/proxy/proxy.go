...
3. If a user already exists in the stack, it won't query the database, and will return
the stored one
4. If the queried user doesn't exist in the stack, it will query the database, remove
...
// UserFinder interface that the database and the Proxy will implement
...
// declare a type so we can implement the UserFinder interface 
type UserList []User
...
func (t *UserList) FindUser(id int32) (User, error) {
	for i := 0; i < len(*t); i++ {
		// (*t) dereferences the underlying array
		if (*t)[i].ID == id {
			return (*t)[i], nil
		}
	}
